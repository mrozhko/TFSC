...
const std::string kTotalAwardAmount = "totalawardamount_";
...
//Obtain a hash of a single block by the height of the data block
...
DBStatus DBReader::GetTotalAwardAmount(uint64_t &Total)
{
    std::string value;
    auto ret = ReadData(kTotalAwardAmount, value);
    if (DBStatus::DB_SUCCESS == ret)
    {
        Total = std::stoull(value);
    }
...
//The program version that initializes the database
...
//Set the total amount of mining
DBStatus DBReadWriter::SetTotalAwardAmount(uint64_t &MiningBalance)
{
    return WriteData(kTotalAwardAmount, std::to_string(MiningBalance));
}
