...
ret trie::insert(nodeptr n, std::string prefix, std::string key, nodeptr value)
{
    ...
    else if (n->name == typeid(shortNode).name())
    {
        ...
        ret r1 = insert(0, prefix + key.substr(0, matchlen + 1), key.substr(matchlen + 1), value);//�����µ�Ҷ�ӽڵ�
    ...
    if (key.length() == 0)
    {
        ...
        if (va->data == vb->data)
        {
            return ret{ false,value,0 };//!bytes.Equal(v, value.(valueNode)), value, nil
        }
...
nodeptr trie::descendKey(std::string key) const
{
    ...
    //std::string const s = virtualDB::get(key);
    dev::bytes bs = dev::fromHex(value);
    if (value == "") return NULL;
    dev::RLP r = dev::RLP(bs);
    //cout << r[0].toString() << endl;
    //cout << r[1][0].toString() << endl;
...
nodeptr trie::decodeFull(std::string hash, dev::RLP const& _r) const
{
    ...
    //16 is not assigned during encoding, so it should be null by default
    /*fn.Children[16] = std::shared_ptr<packing<valueNode>>(
        new packing<valueNode>(valueNode{ _r[16].toString() }));*/
...
nodeptr trie::ToHash(nodeptr n)
{
    ...
    //std::string stringData = evmc::hex(data);
...
nodeptr trie::store(nodeptr n) {
    ...
    else
    {
        ...
        //virtualDB::insert(hash.data, stringData);
