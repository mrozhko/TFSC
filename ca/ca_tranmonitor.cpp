...
void TranMonitor::PrintTxStatus(std::ostream & stream)
{

    std::map<int32_t, std::string> errInfostatus = { };


    for (auto &tx: _TranStatus)
    {
            stream << "---------------------------------------------------------" << std::endl;
            stream << "tx hsah = " << tx.second.Tx.hash() << std::endl;  
            stream << "---------------------------------------------------------" << std::endl;
            ...
            for(auto &i : errInfostatus)
            {
                ...
                {
                        stream << "SelfDoHandle Ret =" << tx.second.LaunchTime.first
                                << "  |  Message =" << i.second 
                                << "  |  Launch Time = " << tx.second.LaunchTime.second << std::endl;
                }
            ...
            if(!tx.second._MonNodeDoHandleAck.empty())
            {
                for(auto &i : errInfostatus)
                {
                    for(auto &ack : tx.second._MonNodeDoHandleAck)
                    {
                        ...
                        {
                            stream << "NodeDoHandleAck Ret = " << ack.first 
                                      << "  |  NodeDoHandleACk Message = " << i.second
                                      << "  |  Time :" << ack.second << std::endl;                     
                        }
                    }
                }
            ...
            stream << "---------------------------------------------------------" << std::endl;
            stream << "Compose Time = " << tx.second._ComposeStatusTime << std::endl;
            stream << "---------------------------------------------------------" << std::endl;
            if(!tx.second._SelfDohandleCount.empty())
            {
                for(auto &i : errInfostatus)
                {
                    for(auto &ack : tx.second._SelfDohandleCount)
                    {
                        if(i.first == ack.first)
                        {
                            stream << "VerifyDoHandleAck Ret = " << ack.first 
                                      << "  |  VerifyDoHandleAck Message = " << i.second
                                      << "  |  Time :" << ack.second << std::endl;                     
                        }
                    }
                }
            ...
            {
                stream << "NOt BroadCast Add block !:";
            }
            else if(tx.second._BroadCastAck.second == 1)
            {
                stream << "Is BroadCast Add block success!" << " | Time = " << tx.second._BroadCastAck.first << std::endl;
            }
            
            stream << "---------------------------------------------------------" << std::endl;
            stream << "Remove pending Time = " << tx.second._VinRemoveTime << std::endl;
            stream << "---------------------------------------------------------" << std::endl;
        
    }

    stream << "_TranStatus in cache size = " << _TranStatus.size() << std::endl;
