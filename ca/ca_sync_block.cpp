...
void SyncBlock::ThreadStart()
{
    ...
    MagicSingleton<Config>::GetInstance()->GetSyncData(syncData);
    
    sync_height_time_ = syncData.interval;
    sync_height_cnt_ = syncData.count;
...
bool SyncBlock::GetFastSyncBlockData(const std::string &send_node_id, const std::vector<FastSyncBlockHashs> &request_hashs, uint64_t chain_height)
{
    ...
    MagicSingleton<BlockHelper>::GetInstance()->AddSyncBlock(fast_sync_block_data, sync_type);
    return true;
}
                                     ...
                                     std::vector<std::string> &ret_node_ids, std::vector<std::string> &req_hashes)
{
    ...
    for (auto &ret_data : ret_datas)
    {
        ...
        for (auto &key : ack.block_hashes())
        {
            ...
            if(start_sync_height == (self_node_height - 10) && end_sync_height == self_node_height)
            {
                MagicSingleton<TranMonitor>::GetInstance()->Add(start_sync_height,end_sync_height,ack.self_node_id(),key);
            }
        }
    }
