...
struct DonAccount
{
    ...
    void CreateTrie(std::string rootHash ,std::string ContractAddr)
    {
        if(rootHash.empty())
        {
    ...
    {
        code = code_;
        const uint8_t* data = reinterpret_cast<const uint8_t*> (code_.c_str());
...
public:
    ...
    std::unordered_map<address, DonAccount> accounts;
...
public:
    ...
    result call(const evmc_message& msg) noexcept override
    {
        ...
        if(value_ll > 0)
        {
            coin_transferrings.push_back({evm_utils::EvmAddrToBase58(msg.code_address) , value_ll});
        ...
        try
        {
            ...
            uint64_t amount = tx_info["deploy_amount"].get<uint64_t>();
            payment.push_back(std::make_pair(*(deployTx.utxo().owner().begin()), amount * dividend_percent));
