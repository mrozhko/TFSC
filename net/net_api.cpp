...
int net_com::input_send_one_message()
{
	...
	for (int i = 0; i < num; ++i)
	{
		...
		if (bl)
		{
			printf("The %d time it was sent successfully\n", i + 1);
		}
		else
		{
			printf("The %d send failed\n", i + 1);
		}
...
int net_com::SendRegisterNodeReq(Node& dest, std::string &msg_id, bool get_nodelist)
{
	...
	//Prohibit the interconnection of multiple intranets and external networks
	std::vector<Node> connect_node_list = MagicSingleton<PeerNode>::GetInstance()->get_nodelist();

	for(auto & node : connect_node_list)
	{
		if(node.public_ip == selfNode.listen_ip)
		{
			DEBUGLOG("SendRegisterNodeReq connect repeat ip = {}", IpPort::ipsz(selfNode.listen_ip));
			return -2;
		}
	}
...
// Create: send node height when it was changed
...
bool net_com::BlockBroadcast_message( BuildBlockBroadcastMsg& BuildBlockMsg, const net_com::Compress isCompress, const net_com::Encrypt isEncrypt, const net_com::Priority priority)
{
	...
	auto getNextNumber=[&](int limit) ->int {
	  	std::random_device seed;//The hardware generates a random number seed
	 	std::ranlux48 engine(seed());
	 	std::uniform_int_distribution<int> u(0, limit-1);
	 	return u(engine);
	};

	//Gets the specified number of random base58addr
	...
	//Determine the size of the nodelist

	const int tosend = global::once_size;

	if(nodelist.size() <=tosend)
	{
		//Send all directly
	...
	}else if(nodelist.size() >=tosend*tosend)
	{
		...
		//Get 50 random subscripts
		std::set<std::string> addrs=getTargetIndexs(tosend,nodelist.size(),nodelist);
		BuildBlockMsg.set_type(1);//Set the number of broadcasts to 1
