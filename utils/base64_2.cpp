...
unsigned long base64_decode(const unsigned char *code, unsigned long code_len, unsigned char *plain)
{
    ...
    for (i = 0; i < code_len; i+=4)
    {
        ...
        {
            plain[j++] = (quad[1]<<4)|(quad[2]>>2); //Take out the last 4 bits of the second character corresponding to the decimal number of the base64 table and combine the first 4 bits of the decimal number of the base64 table for the third character The second character corresponds to the last 4 digits of the decimal number of the base64 table, and the third character corresponds to the first 4 digits of the decimal number of the base64 table
            break;
        }
        else
        {
