...
int Config::_Parse(nlohmann::json json)
{
    ...
    try
    {
        ...
        _syncData.count = json[kCfgSyncData][kCfgSyncDataCount].get<uint32_t>();
        _syncData.interval = json[kCfgSyncData][kCfgSyncDatainterval].get<uint32_t>();

        _threadNum = json[kCfgThreadNumVersion].get<uint32_t>();
...
int Config::GetSyncData(Config::SyncData & syncData)
{
    syncData = _syncData;
    return 0;
}
